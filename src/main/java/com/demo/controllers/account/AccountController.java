package com.demo.controllers.account;
...
//	private AccountValidator accountValidator;
	...
	public String register(ModelMap modelMap) {
		...
		modelMap.put("roles", roleService.limit13());
		//modelMap.put("roles", roleService.findAll());
	    ...
	    if (account.getUsername().equals(authentication.getName())==false) {	
	        try {	 	        	
	            ...
	            if (accountService.save(account)) {       	
	            	String content = "Nhấn vào <a href='http://localhost:8083/account/verify?code=" + securityCode +" &username="+ account.getUsername() +"'>đây</a> để kích hoạt";
	...
	public String forgetpassword(@RequestParam("email") String email, RedirectAttributes redirectAttributes) {
		...
		if (account == null) {
			...
			if (accountService.save(account)) {
				...
				String content = "Nhấn vào <a href='http://localhost:8083/account/updatepassword?code=" + securityCode + "&username=" + account.getUsername() + "'>đây</a> để cập nhật mật khẩu";
